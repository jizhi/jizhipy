...
class PoolFor( object ) : 
	...
	def map_async( self, func, send=None, bcast=None ):
		...
		iterable:
			iterable[0] = [(n1,n2), (Nprocess, rank, pid)]
		...
		send:
			(1) None
			(2) list/tuple. For this case, send[0], send[1], ... are different array, will split send[i] into Nprocess pieces, NOT split send !!!
			(3) 2D-ndarray: send[i] for ranks
		...
		get:
			get=True: pool.map_async(func,iterable).get(10**9)
			get=False: pool.map_async(func,iterable)
		'''
		if (self.zero) : return
		import numpy as np
		from jizhipy.Array import Asarray
		if (self.splitsend) :   # Main here
			...
			for i in range(len(nsl)) : 
				iterable[i] = [[iterable[i], [self.Nmax, i]]]
				sendtmp = ()
		...
		#--------------------------------------------------
			...
			if (send is not None) : 
				...
				for i in range(len(send)) : iterable.append([[[None,None], [Nmax, i]], send[i], bcast])
			else : 
				self.Nprocess = 1
				iterable = [[[[None,None],[1,0]], None, bcast]]
		#--------------------------------------------------
	...
	#		pool = ThreadPool(self.Nprocess)
		...
		for i in range(len(iterable)):
			iterable[i][0][1].append(str(pool._pool[i].pid))
